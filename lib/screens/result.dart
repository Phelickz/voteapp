...
class ResultScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: creatChart(context),
    );
  }

  Widget creatChart(BuildContext context){
    return new charts.BarChart(finalResult(context),
        animate: true,
  ...
  List<charts.Series<VoteData, String>> finalResult(BuildContext context){
    ...
    for (var option in activeVote.options) {
      option.forEach((key, value) {data.add(VoteData(key, value));
      });
    }
    return [
      new charts.Series<VoteData, String>(
        id: 'VoteResult',
        colorFn: (_, pos) {
          if (pos % 2 == 0) {
        ...
        //labelAccessorFn: (VoteData vote, _) => '${vote.option}: ${vote.total}',
      )
    ];
